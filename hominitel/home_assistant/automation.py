...
class Automation(Entity):
    ...
    def update_state(self):
        """Update automation state with memory management"""
        try:
            super().update_state()
            self.is_on = self.state["state"] == "on"
    ...
    def set_state(self, state):
        """Set automation state with memory management"""
        if state not in ["on", "off"]:
            raise ValueError("Invalid state. Use 'on' or 'off'.")

        try:
            self.api.set_state(self.entity_id, "automation", f"turn_{state}")

            # Update local state
            self.is_on = state == "on"
        except Exception as e:
            print(f"Error setting automation state: {e}")
            raise

    def toggle(self):
        """Toggle automation state with memory management"""
        try:
            if self.is_on:
                self.set_state("off")
            else:
